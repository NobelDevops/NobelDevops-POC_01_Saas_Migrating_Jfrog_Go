# example-docker-golang

Example of a Dockerized Golang application.
...
- `git clone git@github.com:PoPETS-AEC/example-docker-golang.git` (SSH)
- `git clone https://github.com/PoPETS-AEC/example-docker-golang.git` (HTTPS)
...
> when the `Dockerfile` or the `go.mod` files are modified.
>
> 1. Pull the Docker image:
> ```bash
> docker pull ghcr.io/popets-aec/example-docker-golang:main
> ```
...
> ```bash
> docker run --rm -it -v ${PWD}:/workspaces/example-docker-golang \
>     -w /workspaces/example-docker-golang \
>     --entrypoint bash ghcr.io/popets-aec/example-docker-golang:main
> ```
...
> ```bash
> docker build -t example-docker-golang:main .
> ```
> 2. Launch the Docker container, attach the current working directory (i.e.,
> run from the root of the cloned git repository) as a volume, set the context
> to be that volume, and provide an interactive bash terminal:
...
> docker run --rm -it -v ${PWD}:/workspaces/example-docker-golang \
>     -w /workspaces/example-docker-golang \
>     --entrypoint bash example-docker-golang:main
> ```
> 3. Execute the example script:
> ```bash
> ./test.sh
